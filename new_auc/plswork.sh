...
output_name="${OUTDIR}${basename_without_extension}_TBCR_similarities_testing_weighed.csv" # final output name to save the dm
...
tbcr_idx_scores = pd.concat([tbcr_idx_scores, pd.DataFrame([[x, x, ${SUMWEIGHTS}.] for x in tbcr_idx_scores.q_index. unique()], columns=['q_index', 'db_index', 'score'])])

# Pivot and do 1 - x / ${SUMWEIGHTS} to get a square distance matrix instead of a similarity matrix with max value SUMWEIGHTS
sim_matrix = (tbcr_idx_scores/${SUMWEIGHTS})
...
sim_matrix = sim_matrix.merge(original_df.set_index('q_index')[extra_cols], left_index=True, right_index=True)
sim_matrix.to_csv(output_filename)
print(f'TBCR simmatrix saved at {output_filename}')
EOF
