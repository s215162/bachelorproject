...
# Setup logging
log_file = "/net/mimer/mnt/tank/projects2/emison/language_model/27_10_esm_processing.log"
logging.basicConfig(
    filename=log_file,
...
datasets = {
    ...
    'not_binders': '/net/mimer/mnt/tank/projects2/emison/language_model/full_sequence_data.csv'
}

# Define output directory
output_dir = "/net/mimer/mnt/tank/projects2/emison/language_model/new_esm_unweighed"
...
df_not_binders = pd.read_csv(datasets['not_binders'])
...
for peptide in peptides:
    for partition in partitions:
        ...
        # Process not binders dataset for various partition combinations
        ...
        for combo in partition_combos:
            df_filtered_not_binders = df_not_binders[(df_not_binders['partition'].isin(combo)) & (df_not_binders['peptide_x'] == peptide)]
            partition_desc = f'partitions_{"_".join(map(str, combo))}'
            logging.info(f"Processing not binders for partitions {partition_desc} and peptide {peptide}")
            process_sequences(df_filtered_not_binders, peptide, partition_desc, 'not_binders')
