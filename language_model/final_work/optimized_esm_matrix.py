...
def process_sequences(df_filtered, peptide, device='cuda' if torch.cuda.is_available() else 'cpu'):
    ...
    all_data = [(row["peptide_x"], row["tcr_full"]) for _, row in df_filtered.iterrows()]
...
def main():
    ...
    output_dir = "/net/mimer/mnt/tank/projects2/emison/language_model/final_work"
