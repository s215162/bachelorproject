...
# Setup logging
log_file = (
    "/net/mimer/mnt/tank/projects2/emison/language_model/27th_Oct_new_matrices/logs/cdr2a_esm_processing.log"
)
logging.basicConfig(
    filename=log_file,
...
output_dir = "/net/mimer/mnt/tank/projects2/emison/language_model/27th_Oct_new_matrices/alpha/cdr2"
...
def process_sequences(df_filtered, peptide, partition_desc, dataset_name):
    ...
    for start in range(0, len(df_filtered), batch_size):
        ...
        data = [(row["peptide_x"], row["CDR2a"]) for _, row in df_subset.iterrows()]
