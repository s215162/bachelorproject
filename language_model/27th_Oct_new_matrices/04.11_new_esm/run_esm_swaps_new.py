...
# Setup logging
log_file = f"/net/mimer/mnt/tank/projects2/emison/language_model/27th_Oct_new_matrices/logs_new/{weight}_cdr_processing.log"
logging.basicConfig(
    filename=log_file,
...
output_dir = f"/net/mimer/mnt/tank/projects2/emison/language_model/27th_Oct_new_matrices/04.11_new_esm/{weight.lower()}"
...
def process_sequences(df_filtered, peptide, weight, dataset_name):
    ...
    for start in range(0, len(df_filtered), batch_size):
        ...
        for i, tokens_len in enumerate(batch_lens):
            sequence_vector = token_representations[i, 1 : tokens_len - 1].sum(0)
            # Sum along the columns to get a single vector for each sequence
            all_sequence_vectors.append(sequence_vector.cpu().numpy())

    # Create DataFrame with raw_index and summed embeddings for export
    output_df = pd.DataFrame(all_sequence_vectors)
    output_df.insert(0, "raw_index", raw_indexes)
    output_file_path = f"/net/mimer/mnt/tank/projects2/emison/language_model/27th_Oct_new_matrices/04.11_new_esm/sequence_summed_vectors_{dataset_name}_{peptide}_{weight}.csv"
