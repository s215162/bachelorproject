...
peptide = "RAKFKQLL"
weight = "TCRb"  # This will be the weight type, e.g., CDR2a

# Setup logging
log_file = (
    "/net/mimer/mnt/tank/projects2/emison/language_model/27th_Oct_new_matrices/logs_new/swaps_"
    + weight + peptide
    + "_cdr_processing.log"
)
logging.basicConfig(
...
output_dir = f"/net/mimer/mnt/tank/projects2/emison/language_model/27th_Oct_new_matrices/04.11_new_esm/12.11/{weight.lower()}"
...
def process_sequences(df_filtered, peptide, weight, dataset_name):
    ...
    for start in range(0, len(df_filtered), batch_size):
        ...
        logging.info(
            f"Processing rows {start} to {end} for {dataset_name}, peptide {peptide}, weight {weight}"
        )
        ...
        for i, tokens_len in enumerate(batch_lens):
            sequence_representations.append(token_representations[i, 1 : tokens_len - 1].sum(0))
    ...
    output_file_path = f"/net/mimer/mnt/tank/projects2/emison/language_model/27th_Oct_new_matrices/04.11_new_esm/12.11/sequence_summed_vectors_{dataset_name}_{peptide}_{weight}.csv"
    output_df.to_csv(output_file_path, index=False)
    logging.info(f"Saved to {output_file_path}")

# Process and save summed vectors for the specified dataset as "swaps"
logging.info(f"Processing dataset for peptide {peptide} with weight {weight}")
process_sequences(df_filtered, peptide, weight, "swaps")
