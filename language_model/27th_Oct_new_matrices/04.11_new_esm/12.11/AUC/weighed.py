...
def merge_dataframes(df1, df2):
    """Merge two DataFrames on 'raw_index' and 'binder' and 'partition."""
    return pd.merge(df1, df2, on=["raw_index", "binder", "partition"], how='outer')

def further_merge(peptide):
    """Merge two CSV datafiles with columns 'raw_index', 'sum_max_similarity', and 'binder' and sum their max_similarity values."""
    ...
    # Save to a new file
    final_filename = f"/net/mimer/mnt/tank/projects2/emison/language_model/27th_Oct_new_matrices/04.11_new_esm/12.11/AUC/{peptide}_Weighed.csv"
...
if len(sys.argv) != 2:
    print("Usage: python weighed.py <peptide>")
    sys.exit(1)
