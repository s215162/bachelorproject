...
# Setup logging
log_file = ("/net/mimer/mnt/tank/projects2/emison/language_model/27th_Oct_new_matrices/logs/beta_full_esm_processing.log")
...
output_dir = "/net/mimer/mnt/tank/projects2/emison/language_model/27th_Oct_new_matrices/beta/full"
...
def process_sequences(df_filtered, peptide, partition_desc, dataset_name):
    ...
    for start in range(0, len(df_filtered), batch_size):
        ...
        data = [(row["peptide_x"], row["TCRb"]) for _, row in df_subset.iterrows()]
        raw_indexes.extend(df_subset["raw_index"].tolist())  # Collect raw_index values
        logging.info(
            f"Processing rows {start} to {end} for {dataset_name}, partition {partition_desc}, peptide {peptide}"
        )
    ...
    output_df.insert(
        0, "raw_index", raw_indexes
    )  # Insert raw_index as the first column
...
for peptide in peptides:
    for partition in partitions:
        ...
        df_filtered_binders = df_binders[
            (df_binders["partition"] == partition)
            & (df_binders["peptide_x"] == peptide)
        ]
        logging.info(
            f"Processing binders for partition {partition} and peptide {peptide}"
        )
        process_sequences(
            df_filtered_binders, peptide, f"partition_{partition}", "binders"
        )
    ...
    for combo in partition_combos:
            df_filtered_swaps = swaps[
                (swaps["partition"].isin(combo)) & (swaps["peptide_x"] == peptide)
            ]
            partition_desc = f'partitions_{"_".join(map(str, combo))}'
            logging.info(
                f"Processing swaps for partitions {partition_desc} and peptide {peptide}"
            )
