...
for peptide in "${peptides[@]}"; do
  CONCATENATED_OUTPUT="${OUTPUTDIRECTORY}/${peptide}_concat"
  ...
  for i in {0..4}; do
    ...
    OUTPUT_FILE="${OUTPUTDIRECTORY}/test_${i}_pred_weighted"

    # Check if the input files exist
    if [[ -f "$TRAIN_FILE" && -f "$TEST_FILE" ]]; then
      echo "Running tbcr_align for partition $i for peptide $peptide..."
      /home/people/morni/tbcr_align/Linux_x86_64/bin/tbcr_align -db $TRAIN_FILE -w "1,1,4,1,1,4" $TEST_FILE > $OUTPUT_FILE
